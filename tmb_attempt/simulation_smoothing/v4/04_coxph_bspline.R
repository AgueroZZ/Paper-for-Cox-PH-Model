...
compile("04_coxph_bspline.cpp")
dyn.load(dynlib("04_coxph_bspline"))

ff <- TMB::MakeADFun(
  data = tmbdat,
  parameters = tmbparams,
  random = "W",
  DLL = "04_coxph_bspline",
  silent = TRUE
)
...
# ### Not parallel:
set.seed(1)
result <- repeating_smoothing_M_times(M = 1, truth = "complicated", N = 1000, baseline = baseline)
# agg_result <- result %>% group_by(method) %>% summarise(rmse = mean(rmse), coverage = mean(coverage), mse = mean(mse))
# agg_result
