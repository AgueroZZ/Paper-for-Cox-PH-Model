...
\gamma_{q}(.) \sim \mathcal{GP}(\sigma_q)
...
\subsection{Modelling Smooth function}\label{subsec:smooth}
...
\end{pmatrix}
\end{equation}
where $\Lambda = DD^{T}$. Expressions for $D$ and $\Lambda^{-1}$ are given in Appendix A. A global intercept $\beta_{0}$ is not estimable in our method.
...
\subcaptionbox{True baseline hazard function}{
	\includegraphics[width=0.45\textwidth,height=3.5in]{true_baseline.png}
}
\subcaptionbox{Estimated baseline hazard function (INLA)}{
	\includegraphics[width=0.45\textwidth,height=3.5in]{inla_baseline.png}
}
\subcaptionbox{Posterior estimate of covariate effect}{
	\includegraphics[width=0.45\textwidth,height=3.5in]{smoothing_simulation.png}
}
\subcaptionbox{Posterior distribution of $\sigma$}{
	\includegraphics[width=0.45\textwidth,height=3.5in]{simulation_hyper.png}
}
...
\subcaptionbox{Posterior effect of \texttt{tpi}}{
  \includegraphics[width=0.45\textwidth,height=3.5in]{leuk_smooth.png}
}
\subcaptionbox{Posterior distribution of $\sigma$}{
	\includegraphics[width=0.45\textwidth,height=3.5in]{leuk_posterior.png}
}
...
\centering
\subcaptionbox{Posterior distribution of $\sigma$}{
	\includegraphics[width=0.5\textwidth,height=4in]{kidney_poster.png}
}
...
The methodology we proposed in this paper provides a flexible way to carry out Bayesian inference for Cox proportional hazard model with partial likelihood, that accommodates the inference on linear covariate effects, semi-parametric covariate effects and frailties. The use of partial likelihood does not require the smoothness assumption on the baseline hazard function, an advantage over INLA. The use of Bayesian inference provides model-based uncertainty quantification of all parameters of interest, an advantage over the frequentist Generalized Additive model approach. We have demonstrated the advantages of our new approach over alternatives through a simulation study and two data analyses. Our proposed method is an appealing option to adopt for the analysis of time-to-event data.
