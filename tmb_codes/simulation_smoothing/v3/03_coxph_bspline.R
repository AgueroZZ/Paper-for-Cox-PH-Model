...
#compile("03_coxph_bspline.cpp")
dyn.load(dynlib("03_coxph_bspline"))
...
save(result, file = "asdsadas.rda")
# ### Not parallel:
# result <- repeating_smoothing_M_times(M = 300, truth = "complicated", N = 1000, baseline = baseline)
# agg_result <- result %>% group_by(method) %>% summarise(rmse = mean(rmse), coverage = mean(coverage), mse = mean(mse))
# agg_result
