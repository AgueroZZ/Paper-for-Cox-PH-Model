...
registerDoMC(ncores)
time_begin <- Sys.time()
result4 <- foreach(i = 1:100,.combine = rbind, .packages = c('foreach', 'stats', 'INLA', 'aghq')) %dopar% do_once(seed = i, beta = beta, N = N, K = 4, sd = sd, bas = "constant")
time_end <- Sys.time()
time_end - time_begin
agg_means <- apply(result4, 2, mean)
agg_table <- matrix(agg_means, ncol=2, nrow = 4, byrow = T)
colnames(agg_table) <- c("AGHQ","INLA")
rownames(agg_table) <- c("beta_cover","beta_mse", "frailty_cover", "frailty_mse")
agg_table
...
#### What if INLA is corrected with its new method? 
...
### Seems like no big change, only improved slightly