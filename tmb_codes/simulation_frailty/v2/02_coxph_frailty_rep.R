...
### N = 2
time_begin <- Sys.time()
result2 <- foreach(i = 1:M,.combine = rbind, .packages = c('foreach', 'stats', 'INLA', 'aghq')) %dopar% do_once(seed = i, beta = beta, N = 2, K = K, sd = sd, bas = "constant")
time_end <- Sys.time()
time_end - time_begin
...
## N = 4
time_begin <- Sys.time()
result4 <- foreach(i = 1:M,.combine = rbind, .packages = c('foreach', 'stats', 'INLA', 'aghq')) %dopar% do_once(seed = i, beta = beta, N = 4, K = K, sd = sd, bas = "constant")
time_end <- Sys.time()
time_end - time_begin
...
## N = 8
time_begin <- Sys.time()
result8 <- foreach(i = 1:M,.combine = rbind, .packages = c('foreach', 'stats', 'INLA', 'aghq')) %dopar% do_once(seed = i, beta = beta, N = 8, K = K, sd = sd, bas = "constant")
time_end <- Sys.time()
time_end - time_begin
...
time_begin <- Sys.time()
time_begin <- Sys.time()
result16 <- foreach(i = 1:M,.combine = rbind, .packages = c('foreach', 'stats', 'INLA', 'aghq')) %dopar% do_once(seed = i, beta = beta, N = 16, K = K, sd = sd, bas = "constant")
time_end <- Sys.time()
time_end - time_begin
agg_means16 <- apply(result16, 2, mean)

### Combine:
aggresult <- rbind(agg_means2,agg_means4, agg_means8, agg_means16)
