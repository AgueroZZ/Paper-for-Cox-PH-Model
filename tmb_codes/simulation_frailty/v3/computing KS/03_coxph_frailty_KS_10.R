...
#### runtime of the proposed method took 29.64428 secs, each sample took 0.003 secs
...
#### Runtime for MCMC is 40.89 minutes; each iteration took 0.07 secs.
#### If run under the same time constraint as the proposed method, can only take 424 iterations.
...
for (i in 1:10000) {
  for (j in 1:60) {
    inla.random.sample[j,i] <- inlasample[[i]]$latent[2071 + j]
  }
}
KS_vec <- c()
for(i in 1:60){
