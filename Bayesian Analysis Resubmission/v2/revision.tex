...
Recently, \cite{ELGM} proposed a fast and scalable methodology for approximate Bayesian inferences for Extended Latent Gaussian Models (ELGM), a broad class of models that includes the Cox PH model with partial likelihood. In their paper, they provided an example in which the approximate Bayesian inference is carried out on partial likelihood for a model that included fixed covariate effects and spatial random effects, and hence demonstrated the possibility of approximate Bayesian inference on partial likelihood with their ELGM type method. Since the method of \cite{ELGM} does not involve noised additional linear predictors in the latent parameter vector, their Hessian matrix of their log-partial likelihood will have fixed size independent of the sample size, and hence is scalable for the analysis of large dataset. 

In this paper, we implemented the approximate Bayesian inference methodologies of \cite{ELGM} on Cox PH models with partial likelihood, and extended the method to include fixed covariate effects, smoothing effect and frailities for correlations between survival times. Through two simulation studies, we illustrated that under certain circumstances, this proposed method based on partial likelihood would yield more reliable result than methods based on full likelihood. To demonstrate the accuracy of the posterior approximation and the computational advantages over existing partial likelihood method based on MCMC, we applied the proposed ELGM type method to re-analyze two datasets of survival times.

The remainder of this paper is organized as follows. In \S\ref{sec:model} we describe the semi-parametric Cox PH model and the existing approach to approximate Bayesian inference. In \S\ref{sec:method}, we describe how to implement approximate Bayesian inference on Cox PH model with partial likelihood using the ELGM type approximation method of \cite{ELGM} and why its inferential results should be preferred in certain settings. In \S\ref{sec:example} we illustrate advantages of the proposed methodology in two simulation studies and through the analysis of Leukaemia survival data analysed by \cite{inlacoxph} and the Kidney catheter data analysed by \cite{kidney}. We conclude in \S\ref{sec:discussion} with a discussion.
...
The semi-parametric covariate effect $\gamma_q$ are modelled as independent zero-mean Gaussian processes defined by their covariance functions $C_{\sigma_{q}}$ which in turn parametrized by $\sigma_q > 0$. A typical choice of covariance function is the 2-fold Integrated Wiener process\citep{wiener}, which has a connection to cubic smoothing splines\citep{wahbaprior}.

To infer the infinite-dimensional parameters $\gamma_{q},q = 1,\ldots,r$, \cite{rw2} proposed the use of second order random walk model (RW2) to approximate the Integrated Wiener process prior, which includes discretizing the covariate $u_q$ into $m_q$ pre-specified bins and approximate each $\gamma_q$ by a piecewise constant function at each bin. The precision matrix of each $m_q$ dimension parameter vector will be denoted as $\Sigma_q^{-1}(\sigma_p)$, which is sparse and available in closed form.
