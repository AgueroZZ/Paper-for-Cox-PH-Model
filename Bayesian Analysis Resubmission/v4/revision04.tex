...
\author{\fnms{Alex} \snm{Stringer}\thanksref{addr3,addr2}\ead[label=e2]{alex.stringer@uwaterloo.ca}}
...
\address[addr3]{Department of Statistical Science,
...
\subfigure[Simple stepwise baseline]{
  \includegraphics[width=0.31\textwidth]{simple_base.png}
}
\subfigure[Oscillating stepwise baseline]{
  \includegraphics[width=0.31\textwidth]{stepwise_base.png}
}
\subfigure[Complicated baseline]{
	\includegraphics[width=0.31\textwidth]{com_base.png}
}
...
\begin{figure}[p]
\centering
\subfigure[MSE for frailty]{
  \includegraphics[width=0.45\textwidth]{sim1_fraMSE.png}
}
\subfigure[Coverage Rate for frailty]{
  \includegraphics[width=0.45\textwidth]{sim1_fraCov.png}
}
\subfigure[MSE for fixed effect]{
  \includegraphics[width=0.45\textwidth]{sim1_betaMSE.png}
}
\subfigure[Coverage Rate for fixed effect]{
  \includegraphics[width=0.45\textwidth]{sim1_betaCov.png}
}
...
\begin{table}[h]
  ...
  \scalebox{0.99}{
  \begin{tabular}{|p{5cm}|
  p{1cm}|p{1cm}|p{2cm}p{2cm}||}
  \hline
  \cline{1-5}
        \multicolumn{1}{l}{} &
        \multicolumn{1}{l}{$\sigma_\xi$} &
        \multicolumn{1}{l}{$\beta$} &
        \multicolumn{2}{c}{$\xi_i$} \\
        \hline
        Number of Measurements & KS  & KS & max KS & mean KS  \\
   \hline
   \hline\hline
...
\begin{figure}[h]
\centering
\subfigure[MSE Comparison]{
  \includegraphics[width=0.45\textwidth]{sim2_box.png}
}
\subfigure[Coverage Rate Comparison]{
  \includegraphics[width=0.45\textwidth]{sim2_bar.png}
}
...
\begin{figure}[h]
  \centering
  \subfigure[Posteriors of $\sigma_\xi$]{
    \includegraphics[width=0.29\textwidth]{kidneyHyper.png}
  }
    \subfigure[Posteriors of $\beta_\text{Age}$]{
    \includegraphics[width=0.29\textwidth]{kid_age.png}
  }
  \subfigure[Posteriors of $\beta_\text{Sex}$]{
    \includegraphics[width=0.29\textwidth]{kid_sex.png}
  }
    \subfigure[Posteriors of $\beta_\text{GN}$]{
    \includegraphics[width=0.29\textwidth]{kid_gn.png}
  }
      \subfigure[Posteriors of $\beta_\text{AN}$]{
    \includegraphics[width=0.29\textwidth]{kid_an.png}
  }
        \subfigure[Posteriors of $\beta_\text{PKD}$]{
    \includegraphics[width=0.29\textwidth]{kid_pkd.png}
  }
  \caption{Posteriors of parameters for the kidney data in section \ref{subsec:kidney}. Posterior distribution obtained using the proposed method(solid line), using MCMC(gray histogram) and using INLA (dashed).}
  \label{fig:kidneyPos}
  \end{figure}
...
Figure \ref{fig:leukFig}(a) shows the posterior mean and posterior 95 \% credible interval of the exponentiated covariate effect of tpi using the proposed approach. To better understand the spatial variation of risk, we simulate $\gamma_2(\boldsymbol{s^*}):= \{\gamma_2(s_i^*), i\in [L]\}$ for a large $L$ from its corresponding posterior predictive distribution, with $s_i^*$ being an arbitrarily chosen point from the region of interest $\mathcal{M}$. This is done by first simulating $\gamma_2(\boldsymbol{s})$ from $\gamma_2(\boldsymbol{s})|\boldsymbol{y}$ using the method described in section \ref{subsec:abi}, and then simulating $\gamma_2(\boldsymbol{s}^*)$ from $\gamma_2(\boldsymbol{s}^*)|\gamma_2(\boldsymbol{s})$ using the method of \cite{schlather2015analysis}. For each $s_i^*$, we simulate 100 independent samples, and their corresponding posterior means and posterior probabilities of having exponentiated effects exceeding 1.5 are shown in Figure \ref{fig:leukFig}(b) and \ref{fig:leukFig}(c).
  ...
  \begin{figure}[p]
    \centering
    \subfigure[$\exp(\gamma_t(\text{tpi}))$]{
      \includegraphics[width=0.45\textwidth,height=2.5in]{leuk_tpi.png}
    }
    \subfigure[Posterior mean of $\gamma_s(s)$]{
      \includegraphics[width=0.45\textwidth,height=2.5in]{leuk_spatial_mean.png}
    }
    \subfigure[Posterior probability of $\gamma_s(s_i) > 1.5$]{
      \includegraphics[width=0.55\textwidth,height=2.5in]{leuk_spatial_prob.png}
    }
...
\begin{table}[h]
  ...
  \begin{tabular}{||c c c c c||}
   ...
   \hline\hline
   m = 1 & 0.906/0.478 & 1.103/1.533 & 0.950/0.926 & 0.038/0.028 \\
   \hline
  m = 2 & 0.918/0.868 & 0.638/0.744 & 0.954/0.936 & 0.018/0.016 \\
   \hline
  m = 3 & 0.934/0.926 & 0.423/0.444 & 0.944/0.944 & 0.011/0.010 \\
   \hline
  m = 4 & 0.939/0.934 & 0.316/0.324 & 0.944/0.944 & 0.008/0.008 \\
   \hline
  m = 5 & 0.941/0.938 & 0.263/0.266 & 0.946/0.944 & 0.006/0.005 \\
   \hline
  m = 10 & 0.943/0.944 & 0.144/0.145 & 0.936/0.938 & 0.003/0.003 \\
    \hline \hline
  \end{tabular}
  }
  \end{center}
  ...
  \begin{table}[h]
    ...
    \begin{tabular}{||c c c c c||}
     ...
     Number of Measurements & $\xi$ Coverage Rate (Proposed/INLA)  & $\xi$ MSE (Proposed/INLA)& $\beta$ Coverage Rate (Proposed/INLA) & $\beta$ MSE (Proposed/INLA)\\
     \hline\hline
     m = 1 & 0.952/0.674 & 0.463/0.569 & 0.954/0.954 & 0.043/0.026 \\
     \hline
    m = 2 & 0.914/0.849 & 0.384/0.421 & 0.952/0.948 & 0.016/0.014 \\
     \hline
    m = 3 & 0.931/0.910 & 0.292/0.308 & 0.950/0.950 & 0.010/0.009 \\
     \hline
    m = 4 & 0.935/0.924 & 0.239/0.248 & 0.952/0.958 & 0.007/0.007 \\
     \hline
    m = 5 & 0.940/0.934 & 0.227/0.233 & 0.932/0.938 & 0.006/0.005 \\
     \hline
    m = 10 & 0.945/0.943 & 0.112/0.114 & 0.952/0.956 & 0.002/0.002 \\
      \hline \hline
    \end{tabular}
    }
    \end{center}
    \caption{Comparison metrics in terms of MSE and posterior coverage rate from 500 independent replications when $\sigma_\xi = 0.8$, for the 60 frailty effects and the fixed effect in the first simulation study in section \ref{subsubsec:sim1}.}
    \label{table:Sim1Agg2}
    \end{table}
    ...
    \begin{table}[h]
      ...
      \begin{tabular}{||c c c c c||}
       ...
       Number of Measurements & $\xi$ Coverage Rate (Proposed/INLA)  & $\xi$ MSE (Proposed/INLA)& $\beta$ Coverage Rate (Proposed/INLA) & $\beta$ MSE (Proposed/INLA)\\
       \hline\hline
       m = 1 & 0.986/0.943 & 0.252/0.145 & 0.948/0.962 & 0.044/0.025 \\
       \hline
      m = 2 & 0.941/0.909 & 0.145/0.137 & 0.954/0.956 & 0.013/0.012 \\
       \hline
      m = 3 & 0.930/0.906 & 0.126/0.125 & 0.954/0.956 & 0.008/0.008 \\
       \hline
      m = 4 & 0.919/0.899 & 0.116/0.117 & 0.944/0.956 & 0.006/0.006 \\
       \hline
      m = 5 & 0.916/0.901 & 0.106/0.107 & 0.928/0.930 & 0.005/0.005 \\
       \hline
      m = 10 & 0.940/0.936 & 0.072/0.073 & 0.954/0.950 & 0.002/0.002 \\
        \hline \hline
      \end{tabular}
      }
      \end{center}
      \caption{Comparison metrics in terms of MSE and posterior coverage rate from 500 independent replications when $\sigma_\xi = 0.4$, for the 60 frailty effects and the fixed effect in the first simulation study in section \ref{subsubsec:sim1}.}
      \label{table:Sim1Agg3}
      \end{table}    
